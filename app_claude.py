...
def get_forehead_params(lm, intensity, img_w):
    ...
    gap        = max(brow_y - eye_y, 10.0)

    # Local forehead band: above brows but below dense hair region.
    y_bottom   = brow_y - gap * 0.10
    y_top      = brow_y - max(g['w'] * 0.24, gap * 1.55)
    y_top      = max(0.0, y_top)
    y_bottom   = max(y_top + 12.0, y_bottom)

    band_h     = y_bottom - y_top
    dy         = -(intensity - 1.0) * band_h * 0.34
